...
さらにライブラリ以外にもモバイルアプリを構成する核となる OS と言語と IDE も毎年大きなアップデートがあり、小さな更新は一年の間にも複数回発生します。特にライブラリは必然的に OS と言語をベースとするので、これらの変更にライブラリは影響を受けやすいです。直近の OS は、およそ一年ごとに新しいバージョンがリリースされています。多少前後することがありますが、 OS は九月リリースが多いようです。モバイルアプリが対応する OS は下限をサポートするほどにより多くのユーザーが利用可能になる一方で、サポートのために最新 OS の機能を取り込みにくいというジレンマが発生します。

https://endoflife.date/ios

![](/images/eof-apple-ios.png)
*End of Life - Apple iOS*

https://endoflife.date/android

![](/images/eof-android-os.png)
*End of Life - Android OS*

言語仕様も同様です。OS と言語は密接に関連するため、特定の OS と言語の組み合わせにも制限が生じます。iOS アプリの開発言語である Swift は IDE の Xcode にバンドルされ、これに依存しています。こちらも近年は九月リリースになっています。Android アプリの開発言語である Kotlin の直近のリリース 1.9.0, 2.0.0 は五月から六月頃となっています。それ以前のバージョンのリリース月にはばらつきがありました[^releases-kotlin-old]。

https://xcodereleases.com/?scope=release

![](/images/releases-xcode.png)
*Releases Xcode*

https://kotlinlang.org/docs/releases.html#release-details

![](/images/kotlin-release-cycle.png)
...
先ほどの kotlinx-serialization-json 1.7.2 のリリースノートの抜粋です。

![](/images/kotlinx-serialization-json-1.7.2.png)
*kotlinx-serialization-json 1.7.2 Release Note*
...
![](/images/serializer-for-kotlin-uuid.png)
*kotlinx-serialization-json 1.7.2 Release Note*

Kotlin 2.0.20 で新しくなったことの紹介ページで、標準ライブラリとして UUID が導入されたことが紹介されていました。筆者は当時このリリースノートをみていたはずなのですが、ライブラリを更新する時点ではすっかり忘れていました。

![](/images/support-for-uuid.png)
...
この件に関しては別の記事としてまとめています。より詳細を知りたい人は下記のリンク先をご覧ください。
...
[^skie-95]: touchlab / SKIE - Issues: Kotlin 2.0.20 incompatibility: NoSuchMethodError: 'org.jetbrains.kotlin.config.CompilerConfigurationKey org.jetbrains.kotlin.backend.konan.KonanConfigKeys$Companion.getBITCODE_EMBED https://github.com/touchlab/SKIE/issues/95
DING_MODE()' 
[^skie-0-9-0]: 執筆時点では、 SKIE は Kotlin 2.0.20 に対応した 0.9.0 がリリースされています。
...
Issues がオープンでなくても、あるいは併用して、 Slack など課題を共有、管理できるツールが提供されている[^jjetbrains-comuunity]こともあり、適宜読み替えてください。
